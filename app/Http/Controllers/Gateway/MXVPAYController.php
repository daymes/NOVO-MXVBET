...
class MXVPAYController extends Controller
{
    use MXVPayTrait;
    ...
    public function callbackMethod(Request $request)
    {
        ...
        if (isset($data['idTransaction'])) {
            ...
            $response = Http::withHeaders([])->post('https://mxvpay.com/api/status-transaction', [
            ...
            if($response->successful()) {
                ...
                if ($responseData['status'] == 'PAID'){
                    if(self::finalizePaymentMXV($data['idTransaction'])) {
                        return response()->json([], 200);
                ...
                if ($responseData['status'] == 'NO_PAID'){
                    \Log::info('NO_PAID');
                    return response()->json([
                        'status' => 'NO_PAID',
                    ]);
    ...
    {
        return self::consultStatusTransactionMXV($request);
    }
