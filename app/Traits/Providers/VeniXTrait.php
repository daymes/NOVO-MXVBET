...
trait VeniXTrait
{
     ...
     * @var string
     */
    public static $apiUri = 'https://voxelgator.com/api/v1/';
    ...
    {
        \Log::info('WebhookVeniX '. json_encode($request->all()));
    ...
    {
        \Log::info('GetBalanceVeniX request received', ['user_id' => $request->user_id]);
        
        $walletResp = Wallet::where('user_id', $request->user_id)->first();
        
        if ($walletResp) {
            \Log::info('User balance retrieved', [
        ...
        \Log::warning('Invalid user ID for balance check', ['user_id' => $request->user_id]);
